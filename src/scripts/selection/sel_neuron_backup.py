...
def set_path(args) -> Path:
    ...
    all_filename = f"{args.data_range_end}_all.debug" if args.debug else f"{args.data_range_end}_all.csv"
    sel_dir = (
    ...
    return sel_dir/ filename, sel_dir/ all_filename


def extract_neurons():
    """Extarct neurons and delta losses given the """
...
def main() -> None:
    ...
    save_path,all_path = set_path(args)
    if save_path.is_file():
        logger.info(f"{save_path} already exists, skip!")
        sys.exit(0)
    if all_path.is_file() or :
        logger.info(f"Extracting top {args.top_n} from {save_path}")
