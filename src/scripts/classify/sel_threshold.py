...
from neuron_analyzer.classify.sel_threshold import NeuronFeatureExtractor, SingleThresholdCalculator
...
def configure_path(args):
    ...
    filename = f"{args.data_range_end}_{args.top_n}{filename_suffix}"
    save_path = settings.PATH.classify_dir / "data" / args.vector / args.model / save_heuristic / filename
    save_path.parent.mkdir(parents=True, exist_ok=True)
    abl_path = settings.PATH.result_dir / "ablations" / args.vector / args.model
    return save_path, abl_path


def compute_threshold(args, device: str, abl_path: Path, save_path: Path, stat_results: dict, step_dirs: list) -> dict:
    """Extract optimal threshold across multiple steps."""
    for step in step_dirs:
        # Initialize the extractor
        extractor = NeuronFeatureExtractor(
            args=args, abl_path=abl_path, step_path=step[0], device=device, out_dir=save_path, resume=args.resume
        )
        data = extractor.save_results()
        calculator = SingleThresholdCalculator(data=data, out_dir=save_path)
...
def main() -> None:
    ...
    abl_path = settings.PATH.result_dir / "ablations" / args.vector / args.model
    save_path = configure_path(args)

    # initilize with the step dir
    step_processor = StepPathProcessor(abl_path)
    _, step_dirs = step_processor.resume_results(args.resume, save_path)

    if save_path.is_file():
        logger.info(f"{save_path} already exists, skip!")
    else:
        # Process multiple steps
        compute_threshold(args, save_path, abl_path)
