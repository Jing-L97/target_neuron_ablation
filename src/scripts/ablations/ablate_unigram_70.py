...
def process_single_step(args: DictConfig, step: int, save_path: Path) -> None:
    ...
    This function has been enhanced to support different ablation modes:
    - "mean": Traditional mean ablation across all tokens (original behavior)
    - "longtail": Apply ablation only to long-tail tokens based on frequency threshold
    ...
    longtail_threshold = getattr(args, "longtail_threshold", 0.001)  # Default threshold
