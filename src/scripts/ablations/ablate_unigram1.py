...
class NeuronAblationProcessor:
    ...
    def __init__(
        ...
        Args:
            args: Configuration for the ablation process
            settings: Application settings containing necessary paths
    ...
    def load_unigram(self) -> torch.Tensor:
        ...
        if "pythia" in self.args.model:
            ...
            unigram_distrib = get_pile_unigram_distribution(
                ...
                file_path=self.settings.PATH.unigram_dir / "pythia-unigrams.npy"
            )
        elif "gpt" in self.args.model:
            self.logger.info("Loading unigram distribution for gpt2...")
            unigram_distrib = get_pile_unigram_distribution(
                device=self.args.device,
                file_path=self.settings.PATH.unigram_dir / "gpt2-small-unigrams_openwebtext-2M_rows_500000.npy",
    ...
    def get_tail_threshold(self, ablation_mode: str, save_path: Path) -> tuple[t.Optional[float], t.Optional[dict]]:
        """Calculate threshold for long-tail ablation mode."""
        if ablation_mode == "longtail":
            if self.unigram_distrib is None:
                self.load_unigram()
                
            window_size = 2000
            analyzer = ZipfThresholdAnalyzer(self.unigram_distrib, window_size=window_size)
    ...
    def process_single_step(self, step: int, save_path: Path) -> None:
        ...
        self.load_model_and_tokenizer(step)
        
        # Ensure unigram distribution is loaded
        if self.unigram_distrib is None:
            self.load_unigram()
        ...
        tokenized_data = utils.tokenize_and_concatenate(first_1k, self.tokenizer, max_length=256, column_name="text")
        ...
        if self.args.neuron_range is not None:
            ...
            all_neuron_indices = list(range(0, self.model.cfg.d_mlp))
        ...
        entropy_dim_layer = self.model.cfg.n_layers - 1
        ...
        ablation_mode = getattr(self.args, "ablation_mode", "mean")  # Default to 'mean' if not specified
        self.logger.info(f"Using ablation mode: {ablation_mode}")
        
        # Get threshold for longtail mode
        longtail_threshold, threshold_stats = self.get_tail_threshold(ablation_mode, step, save_path)
        
        if ablation_mode == "longtail" and longtail_threshold is not None:
            ...
            self.logger.info(
                f"Long-tail tokens: {long_tail_count} "
                f"({long_tail_count / vocab_size * 100:.2f}% of vocabulary)"
            )
        ...
        self._save_results(results, ablation_mode, longtail_threshold, threshold_stats, step, save_path)

    def load_model_and_tokenizer(self, step: int) -> tuple[t.Any, t.Any]:
        """Load model and tokenizer for processing."""
        # Load HF token
        with open(self.settings.PATH.unigram_dir / self.args.hf_token_path, "r") as f:
            hf_token = f.read()
        ...
        model, tokenizer = load_model_from_tl_name(
            ...
            self.device, 
            step=step,
            cache_dir=self.settings.PATH.model_dir, 
            hf_token=hf_token
        )
    ...
    def _save_results(
        ...
        results: dict, 
        ablation_mode: str, 
        longtail_threshold: t.Optional[float],
        threshold_stats: t.Optional[dict],
        step: int, 
        save_path: Path
    ) -> None:
        ...
        Args:
            ...
            ablation_mode: Type of ablation used
            longtail_threshold: Threshold for longtail ablation
            threshold_stats: Statistics for threshold calculation
            step: Current processing step
            save_path: Path to save results
        """
        ...
        if ablation_mode == "longtail" and threshold_stats is not None:
        ...
        output_path = save_path / f"k{self.args.k}_{ablation_mode}.feather"
...
def get_tail_threshold(ablation_mode,):
