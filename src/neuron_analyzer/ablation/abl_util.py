...
def get_entropy_activation_df(
    ...
    if residuals_dict is None:
        residuals_dict = {"resid_pre": [], "resid_mid": [], "resid_post": []}
    ...
    for attn_head_name in bos_attn_heads_list:
        if attn_head_name not in attention_to_bos_dict:
    ...
    for i in tqdm.tqdm(range(num_batches)):
        ...
        end = min(end, len(tokenized_data))
        ...
        rank_of_correct_token = np.concatenate(
            (rank_of_correct_token, np.zeros((rank_of_correct_token.shape[0], 1))), axis=1
        )
        ...
        if model2_for_kl is not None:
            ...
            kl_divergence = torch.nn.functional.kl_div(
                logits.log_softmax(dim=-1), model2_logits.log_softmax(dim=-1), log_target=True, reduction="none"
            ).sum(dim=-1)
        ...
        if unigram_distrib is not None:
            ...
            kl_divergence = kl_div(
                single_token_probs, unigram_distrib.log().cpu(), reduction="none", log_target=True
            ).sum(dim=-1)
        ...
        for neuron_name in neuron_names:
            ...
            neuron_activations_cache_dict[neuron_name].append(
                cache[tl_utils.get_act_name("post", neuron_layer)][..., neuron_index].cpu().numpy()
            )

            if cache_pre_activations:
                neuron_pre_activations_cache_dict[neuron_name].append(
                    cache[tl_utils.get_act_name("pre", neuron_layer)][..., neuron_index].cpu().numpy()
                )
        ...
        if cache_residuals:
            for act_name in residuals_dict.keys():
                ...
                if apply_ln_to_cache:
                    activation = cache.apply_ln_to_stack(
                        activation.to(device), layer=apply_ln_location_layer, mlp_input=apply_ln_location_mlp_input
                    )

                residuals_dict[act_name].append(activation.cpu().numpy())

        # attention to bos
        for attn_head in attention_to_bos_dict:
            pattern = r"L(\d+)H(\d+)"
            ...
            attention_to_bos_dict[attn_head].append(
                cache[tl_utils.get_act_name("pattern", layer_idx)][:, head_idx, :, 0].cpu().numpy()
            )
    ...
    for neuron_name in neuron_activations_cache_dict:
        neuron_activations_cache_dict[neuron_name] = np.concatenate(neuron_activations_cache_dict[neuron_name], axis=0)

        if cache_pre_activations:
            neuron_pre_activations_cache_dict[neuron_name] = np.concatenate(
                neuron_pre_activations_cache_dict[neuron_name], axis=0
            )
    ...
    token_df["correct_token_rank"] = token_df[
        "rank_of_correct_token"
    ]  # same as above, because some scripts use one name and some use the other
    ...
    if cache_residuals:
        for act_name in residuals_dict.keys():
            residuals_dict[act_name] = np.concatenate(residuals_dict[act_name]).reshape(
                len(token_df), model.cfg.d_model
            )
...
def filter_entropy_activation_df(entropy_df, model_name=None, tokenizer=None, start_pos=3, end_pos=-1):
    ...
    filtered_entropy_df = filtered_entropy_df[
        filtered_entropy_df["pos"].between(start_pos, end_pos_val, inclusive="left")
    ]
...
def bos_ablate_attn_heads(
    ...
    if compute_resid_norm_change:
        ...
        assert f"{model.cfg.n_layers - 1}.resid_post_norm" in entropy_df.columns, (
            "To compute resid norm change, entropy_df must have column final_layer.resid_post_norm in entropy_df"
        )
    ...
    for i, r in new_entropy_df.iterrows():
        ...
        for attn_head in attn_head_names:
            ...
            hooks.append(
                (tl_utils.get_act_name("pattern", layer_idx), partial(ablation_hook, position=position, head=head_idx))
            )
        ...
        with model.hooks(fwd_hooks=hooks):
            ...
            if compute_resid_norm_change:
                ...
                post_ablation_resid_norm.append(
                    ablated_cache[tl_utils.get_act_name("resid_post", model.cfg.n_layers - 1)][:, position]
                    .norm(dim=-1)
                    .cpu()
                    .numpy()
                )  # hard coded for final layer

        for neuron_name in neuron_names:
            neuron_layer = int(neuron_name.split(".")[0])
            neuron_index = int(neuron_name.split(".")[1])

            post_ablation_neuron_activations_cache_dict[neuron_name].append(
                ablated_cache[tl_utils.get_act_name("post", neuron_layer)][..., position, neuron_index].cpu().numpy()
            )

            if cache_pre_activations:
                post_ablation_neuron_pre_activations_cache_dict[neuron_name].append(
                    ablated_cache[tl_utils.get_act_name("pre", neuron_layer)][..., position, neuron_index].cpu().numpy()
                )
        ...
        post_ablation_top_ps.append(
            np.log(ablated_logits.softmax(dim=-1).max(dim=-1).values.cpu().numpy()[:, position])
        )
    ...
    new_entropy_df["ln_final_scale_diff"] = (
        new_entropy_df["post_ablation_ln_final_scale"] - new_entropy_df["ln_final_scale"]
    )

    for neuron_name in post_ablation_neuron_activations_cache_dict:
        new_entropy_df[f"{neuron_name}_activation_post_abl"] = np.concatenate(
            post_ablation_neuron_activations_cache_dict[neuron_name], axis=0
        ).flatten()

        if cache_pre_activations:
            new_entropy_df[f"{neuron_name}_pre_activation_post_abl"] = np.concatenate(
                post_ablation_neuron_pre_activations_cache_dict[neuron_name], axis=0
            ).flatten()

    if compute_resid_norm_change:
        new_entropy_df[f"post_ablation_{model.cfg.n_layers - 1}.resid_post_norm"] = np.concatenate(
            post_ablation_resid_norm, axis=0
        )
        new_entropy_df[f"{model.cfg.n_layers - 1}.resid_post_norm_change"] = (
            new_entropy_df[f"post_ablation_{model.cfg.n_layers - 1}.resid_post_norm"]
            - new_entropy_df[f"{model.cfg.n_layers - 1}.resid_post_norm"]
        )

    return new_entropy_df


def mean_ablate_attn_heads(
    attn_head_names,
    tokenized_data=None,
    entropy_df=None,
    model=None,
        ...
        for attn_head in attn_head_names:
            ...
            hooks.append(
                (
                    ...
                    partial(
                        mean_ablation_hook, position=position, head=head_idx, attn_z=ablation_values_dict[attn_head]
                    ),
                )
            )
...
def generate_induction_examples(
    ...
    if use_natural_text:
        ...
        def tok_function(examples):
            return {
                "tokens": tokenizer(
                    examples["text"], add_special_tokens=False, max_length=max_len, truncation=True
                ).input_ids
            }
        ...
        sequences = torch.randint(
            0, model.cfg.d_vocab, size=(num_examples, seq_length), generator=torch_seed, device=device
        )
    ...
    if use_separator:
        separator = einops.repeat(
            model.to_tokens(use_separator, prepend_bos=False), "1 seq -> num_ex seq", num_ex=num_examples
        )
...
def get_induction_data_and_token_df(
    model,
    tokenizer,
    seq_length,
    num_examples,
    ...
    artificial_data = Dataset.from_dict(
        {
            "tokens": generate_induction_examples(
                model,
                tokenizer,
                ...
                seed=seed,
                device=device,
                use_natural_text=use_natural_text,
...
def generate_unigram_df(data, model, tokenizer, model_name, save_file=False, save_path="datasets/unigram_df.pkl"):
    def tokenization(example):
        return tokenizer(
            example["text"], truncation=False, padding=False, add_special_tokens=False, return_attention_mask=False
        )
    ...
    if "pythia" in model_name:
        vocab_dict = dict.fromkeys(range(model.cfg.d_vocab), 0)
    else:
        vocab_dict = dict.fromkeys(tokenizer.get_vocab().values(), 0)
    ...
    filtered_ngrams = Counter(
        {
            ...
            if (len(set(k).intersection(banned_tokens)) == 0) and (len(set(k)) == len(k)) and (v > 1)
        }
    )
...
def get_banned_tokens_for_induction(model, tokenizer):
    unknown_token_list = [
        i for i in range(model.cfg.d_vocab) if "�" in model.to_single_str_token(i)
    ]  # this covers cases like 447 and 227 which are jointly tokenised as an apostrophe in gpt2-small
...
def get_natural_induction_data(
    tokenized_data,
    tokenizer,
    induction_prefix_length=4,
    ...
    filtered_data = tokenized_data.filter(
        lambda example: len(example["tokens"][example["tokens"] == tokenizer.bos_token_id]) < 2
    )

    # add info about induction (used to filter out non-induction samples)
    filtered_data = filtered_data.map(
        lambda x: add_induction_info(x, n=induction_prefix_length, banned_tokens=banned_tokens), batched=False
    )
    ...
    filtered_data = filtered_data.filter(
        lambda example: example["induction_ngram_count"] <= max_induction_ngram_count, batched=False
    )
    # making sure there are tokens in between repetitions of induction prefix
    filtered_data = filtered_data.filter(
        lambda example: example["n_tokens_between_induction_ngrams"] >= min_distance_between_induction_ngrams,
        batched=False,
    )
...
def get_potential_entropy_neurons_udark(
    model,
    select_mode="top_n",
    ...
    df = pd.DataFrame(
        {
            "norm_fraction_on_U_entropy": norm_fraction_on_U_entropy.cpu(),
            "norm": norm.cpu(),
            "neuron_index": np.arange(model.cfg.d_mlp),
        }
    )
    ...
    if plot_graph:
        fig = px.scatter(
            df,
            x="norm",
            y="norm_fraction_on_U_entropy",
            title=f"Entropy neurons, {len(top_percent_neuron_names)}",
            hover_name="component_name",
        )

        min_thresold_value = df[df["component_name"] == top_percent_neuron_names[-1]][
            "norm_fraction_on_U_entropy"
        ].item()
